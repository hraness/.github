...
- Distribute the `hraness-public-copy` block below into repository `AGENTS.md` files the same way as the `hraness-delivery` block. Distribute the `hraness-articles` block the same way into repositories whose sites publish essays or blog posts. This file is the source of both blocks: change the text here first, then copy it verbatim between the markers.
